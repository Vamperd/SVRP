...
此外，在真实物流系统中，车辆行驶时间往往并非完全确定。道路拥堵、交通事故、天气变化和城市高峰期出行等因素都会导致实际行驶时间相对规划时间发生偏移。对于带时间窗约束的配送问题而言，行驶时间扰动会直接影响客户到达时刻，进而可能导致原本在静态条件下可行的路线在执行过程中出现时间窗违约。因此，仅研究确定性 TWVRP 仍然难以完整反映真实调度系统中的执行风险。近期 Heakl 等~\cite{heakl2025svrpbench} 提出的 SVRPBench 基准表明，现有学习型求解器在随机交通分布偏移下可行率衰减超过 20\%，凸显了在动态环境下评估路径鲁棒性的必要性。基于此，本文在标准 TWVRP 求解的基础上，进一步引入交通扰动评估，用于分析静态规划路线在随机交通环境下的鲁棒性退化，并比较不同类型求解方法在解质量、运行效率和动态鲁棒性方面的差异。
...
本文采用 Solomon~\cite{solomon1987algorithms} 与 Homberger~\cite{homberger1999two} 标准 TWVRP 数据集作为实验基准，覆盖 100、200、400、600、800 和 1000 客户规模。该类数据集包含客户坐标、需求量、时间窗、服务时间、车辆容量等信息，能够较好地用于验证不同求解算法在标准路径规划问题中的性能。围绕该问题，本文分别实现和分析三类求解器：基于启发式规则与局部搜索的启发式求解器、基于 OR-Tools 的约束优化求解器，以及基于神经组合优化和强化学习的学习型求解器。
...
其中，$\sigma_{traffic}$ 表示交通扰动标准强度，$\lambda$ 表示扰动强度倍数，$\varepsilon$ 为服从标准正态分布的随机变量，$\left(1-e^{-d_{ij}/50}\right)$ 为距离相关放大项~\cite{heakl2025svrpbench}。该项表示长距离边更容易受到交通不确定性的影响，而短距离边受到的扰动相对较小。为避免扰动后行驶时间出现不合理取值，实际仿真中设置如下下界：
...
本文采用 Solomon~\cite{solomon1987algorithms} 与 Homberger~\cite{homberger1999two} 标准 TWVRP 数据集作为实验基准。Solomon 数据集~\cite{solomon1987algorithms} 主要用于 100 客户规模的 VRPTW 测试，Homberger 数据集~\cite{homberger1999two} 在 Solomon 数据集基础上扩展到 200、400、600、800 和 1000 客户规模，适合用于检验算法在大规模带时间窗车辆路径问题中的扩展能力。
...
最近邻算法是一类典型的构造型启发式方法，基本思想是从配送中心出发，在尚未访问的客户中选择与当前节点距离较近的客户，并不断将其加入当前车辆路线。当现有车辆无法继续服务其他客户时，关闭当前路线并启用下一辆车辆。然而，单纯使用最近邻规则容易产生局部贪心现象：算法可能在路线前段连续选择距离较近的客户，但在路线末端被迫连接一个较远节点，从而产生交叉路径或较长的回程边。因此，任务在最近邻构造后增加 2-opt~\cite{lin1973effective} 局部搜索。2-opt 每次从同一条路线中选择两条边，反转两条边之间的客户访问顺序，并在新路线更优时接受该操作。通过反复执行该过程，可以消除部分路径交叉并缩短单条路线的行驶距离。
...
\end{figure}
\subsection{基础禁忌搜索算法}
禁忌搜索（Tabu Search）由 Glover~\cite{glover1989tabu,glover1990tabu} 提出，是一种具有记忆机制的局部搜索方法。传统局部搜索通常只接受能够改善当前解的邻域动作，因此容易停留在局部最优解。禁忌搜索允许算法在部分迭代中接受不优于当前解的动作，并通过禁忌表暂时禁止最近执行过的动作，从而避免算法立即返回此前解，在更大的解空间内进行探索。本节在 NN+2-opt 算法的初始解基础上，通过客户移动和客户交换生成邻域解。
...
为了突破基础禁忌搜索在中大规模实例中成本改善有限的问题，实验在原有 Relocate 和 Swap 邻域的基础上，引入能够进行跨路线结构调整的 2-opt* 操作~\cite{potvin1995exchange}，并结合长边引导、近邻候选集、候选动作预筛选以及局部强化等机制，构建融合 2-opt* 的改进禁忌搜索算法。
\subsubsection{算法思路}
...
前文已经对 TWVRP 的问题定义、数学模型、数据集结构和评价指标进行了说明。本节进一步介绍基于 OR-Tools 的 TWVRP 求解器设计方法。OR-Tools~\cite{perron2023ortools} 是 Google 开源的组合优化工具包，其中 Routing Library 能够对车辆路径问题、带时间窗车辆路径问题和容量约束车辆路径问题进行建模求解。与启发式算法和强化学习方法相比，OR-Tools 的优势在于约束建模清晰、工程实现成熟、可行性保障能力较强，适合作为本文标准 TWVRP 问题的确定性优化基线。
...
表 \ref{tab:prop_degradation} 展示了比例噪声下静态模型的系统性退化：可行率从 600 规模的约 58\% 崩塌至 1000 规模的约 36\%。这并非静态网络设计的失败，而是预规划路线在不具备在线调整能力时，其初始等待余量不足以吸收长距离边的大幅度行程时间波动。这一退化趋势与第~\ref{sec:ortools}~节中 OR-Tools 静态路线在比例扰动下的表现一致，也与第~\ref{sec:heuristic}~节中三种启发式算法混合模式下的退化特征相符，说明交通不确定性导致的执行失效是静态预规划方法的共性挑战。近期 SVRPBench~\cite{heakl2025svrpbench} 在对 POMO 和 Attention Model 的评测中也观察到相同现象，表明 RL 求解器在分布偏移下的脆弱性是跨模型、跨基准的普遍问题。

值得注意的是，前文在启发式算法和 OR-Tools 框架下已分别实现了离线鲁棒预规划：采用 0.75 分位数交通矩阵（启发式交通模式）或 mean+2$\sigma$ 保守矩阵（OR-Tools）进行重新规划。实验表明这些手段能够显著降低时间窗违约率，但存在三个结构性局限，使得离线预规划难以在保持经济性的前提下应对大规模交通扰动：

\textbf{第一，信息利用率低。}离线预规划将整个随机交通分布压缩为一个确定性矩阵，抛弃了分布的方差和协方差信息。即使采用 P90 或 mean+2$\sigma$ 等保守估计，当路线条数较多时，所有路线同时可行的联合概率依然趋近于零。以本文 1000 客户实例为例，即使每条路线在保守矩阵下单独可行概率较高，约 100 条路线同时可行的联合概率仍可忽略不计——这是“单矩阵预规划”范式的固有天花板，而非参数调节不足。

\textbf{第二，缓冲分配盲目。}离线保守规划对所有边施加同质的行驶时间放大，本质上是一种盲缓冲策略。然而在实际路线执行中，前序客户处的早到等待已天然充当了时间缓冲，部分路段的扰动可以被吸收而不产生实际违约；真正致命的扰动往往集中在路线末段的少数长距离边上，此前的缓冲已被沿途消耗殆尽。离线方法无法区分这两种情形，导致大量缓冲被浪费在已被等待时间覆盖的路段，而真正的尾部风险仍暴露不足。

\textbf{第三，缺乏恢复与协同机制。}离线规划的路线一旦确定，车辆间的客户分配和访问顺序即被固定。前序延误只能沿单一路线向后传播，无法通过跨车辆的负载均衡（将客户从延误车辆转移至空闲车辆）、访问顺序重排（跳过时间窗紧迫的客户先服务其他客户）或在线放弃—补偿决策来阻断传播链。在 1000 客户、100 辆车并发的大规模调度场景中，离线预规划等同于放弃了对这些协同机制的利用。

上述三个局限在本文实验数据中有直接体现。第~\ref{sec:heuristic}~节混合模式中，NN+2-opt 与基础禁忌搜索在 1000 客户下的场景可行率降至 0\%；改进禁忌搜索虽通过更短、更集中的路线将平均 CVR 从 31.64\% 降至 30.73\%，但场景可行率仍未改善。第~\ref{sec:ortools}~节中，400 客户规模的鲁棒预规划以 33.6\% 的行驶成本增长和 11 辆车的额外投入，仅将可行概率从 0.0\% 提升至 46.7\%。这些结果一致表明，仅依靠离线保守规划无法实现大规模交通扰动下的高可行率调度——鲁棒性的根本瓶颈不在于路线“更短”或规划矩阵“更保守”，而在于路线执行过程中缺乏对实际交通状态的感知与反馈。

基于上述分析，本章提出静态引导的动态追索方法：利用已训练好的静态规划器批量生成高质量基准路线，在交通扰动和时间窗漂移条件下构造动态训练数据，训练出一个具备全局结构感知能力的在线序贯决策网络，在路线执行过程中根据实时交通状态进行自适应调度。
